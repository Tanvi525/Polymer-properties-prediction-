...
st.markdown('<p class="sub-header">AI-Powered Polymer Property Prediction • Instant Results • Research Grade</p>', unsafe_allow_html=True)
...
# Prediction function (replace with your actual trained model)
    ...
    if st.button("🚀 Predict Polymer Properties", type="primary"):
        if smiles_input:
            with st.spinner("🔄 Analyzing molecular structure..."):
                # Compute descriptors
                descriptors = compute_molecular_descriptors(smiles_input)
                ...
                if descriptors:
                    ...
                    with st.expander("🔍 View Molecular Analysis"):
                        ...
                        with insight_cols[1]:
                            ...
                            if descriptors['NumAromaticRings'] > 0:
                                st.info("🔸 Contains aromatic rings - likely higher Tg")
                            if descriptors['NumRotatableBonds'] > 5:
                                st.info("🔸 Flexible chain - likely lower Tg")
                            if descriptors['NumHDonors'] > 0:
                                st.info("🔸 Polar groups present - affects dielectric")
                else:
...
with info_cols[0]:
    ...
    **🔬 How It Works:**
    ...
    2. **Compute** 200+ molecular descriptors
    3. **AI Model** predicts properties using Random Forest
...
with st.expander("🚀 Deploy Your Own Version"):
    st.markdown("""
    **To deploy this website:**
    ...
    4. **Your website will be available at:**
       `https://yourname-polymer-predictor.streamlit.app/`
